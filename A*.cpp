...
{
   long p=0,i,j,i1,j1;
   point val= { COM[0][valr] , COM[1][valr] };
   int endx=COM[0][valg],endy=COM[1][valg];
   dist[val.x][val.y]=0;
   fn[val.x][val.y]=dist[val.x][val.y]+sqrt((endx-val.x)*(endx-val.x) +(endy-val.y)*(endy-val.y));
   while(p<traverse)
   {
       p++;
       int min=infinity;
          ...
          for (j1=-1;j1<=1;j1++)
          {
             if (isvalid(binary,val.x+i1,val.y+j1)==1)
             {
	             ...
	             if (mini < dist[val.x + i1][val.y + j1])
	                 { 
                                 double gn=sqrt((endx-val.x-i1)*(endx-val.x-i1) + (endy-val.y-j1)*(endy-val.y-j1));
		                 dist[val.x + i1][val.y + j1] = mini;
                                 fn[val.x + i1][val.y + j1]=mini+gn;
